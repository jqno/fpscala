...
import nl.jqno.fpscala.ch13_iomonad.{IO,Monad,Free,unsafePerformIO}
...
object SimpleStreamTransducers {
  ...
  sealed trait Process[I,O] {
    ...
    def |>[O2](p2: Process[O,O2]): Process[I,O2] = p2 match {
      ...
      case Await(recv) => this match {
        ...
        case Emit(h2, t2) => t2 |> recv(Some(h2))
        case Await(recv2) => Await(i => recv2(i) |> p2)
      }
    }
    // case class Emit[O,O2](head: O2, tail: Process[O,O2] = Halt[O,O2]())
    //   extends Process[O,O2]

    // case class Await[O,O2](recv: Option[O] => Process[O,O2])
    //   extends Process[O,O2]
  ...
  object Process {
    ...
    def takeWhile[I](f: I => Boolean): Process[I,I] = await { d =>
      if (f(d))
        emit(d, takeWhile(f))
      else
        Halt()
    ...
    def dropWhile[I](f: I => Boolean): Process[I,I] = await { d =>
      if (f(d))
        dropWhile(f)
      else
        emit(d, dropWhile(f))
    ...
    def mean: Process[Double,Double] = {
      def go(total: Double, count: Int): Process[Double,Double] = await { d =>
        val newTotal = total + d
        val newCount = count + 1
        val mean = newTotal / newCount
        emit(mean, go(newTotal, newCount))
      }
      go(0.0, 0)
    }
    ...
    def sum2: Process[Double,Double] = loop(0.0) { case (i, s) => (s + i, s + i) }

    def count3[I]: Process[I,Int] = loop(0) { case (i, s) => (s + 1, s + 1) }
    ...
    def exists[I](f: I => Boolean): Process[I,Boolean] =
      loop(false) { case (i, b) => (b || f(i), b || f(i)) }
     ...
     */
    def temperatureConverter: Process[String,String] =
...
object GeneralizedStreamTransducers {
  ...
  object Process {
    ...
    import nl.jqno.fpscala.ch13_iomonad.IO0.fahrenheitToCelsius
